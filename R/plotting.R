...
create.haplotype.plot <- function(chrom.position, points.blue, points.red, x.min, x.max, title, xlab, ylab, point.size = 1.5, cytoband_data) {
  ...
  for (i in seq_len(nrow(cytoband_data))) {
    ...
    band_name <- cytoband_data$band[i]

    plot <- plot +
      geom_vline(xintercept = c(band_start, band_end), linetype = "dashed", color = "grey", size = 0.3) +
      annotate("text", x = band_mid, y = 1.05, label = band_name, angle = 0, vjust = -0.5, hjust = 0.5, size = 3, color = "black") +
  ...
  create.subclonal.cn.plot = function(chrom, chrom.position, LogRposke, LogRchr, BAFchr, BAFsegchr, BAFpvalschr, subcloneres, siglevel, x.min, x.max, title, xlab, ylab.logr, ylab.baf, breakpoints_pos = NULL, svs_pos = NULL) {
    ...
    for (i in 1:dim(subcloneres)[1]) {
      if (subcloneres[i, 1] == chrom) {
        text((as.numeric(subcloneres[i, "startpos"]) + as.numeric(subcloneres[i, "endpos"])) /
               2 /
               1000000, as.numeric(subcloneres[i, "BAF"]) - 0.04,
        ...
        if (!is.na(subcloneres[i, "nMaj2_A"])) {
          text((as.numeric(subcloneres[i, "startpos"]) + as.numeric(subcloneres[i, "endpos"])) /
                 2 /
                 1000000, as.numeric(subcloneres[i, "BAF"]) - 0.08,
  ...
  create.bb.plot.subclones = function(bafsegmented, subclones, ploidy, rho, goodnessOfFit, pos_min, pos_max, subcl_min, subcl_max, is_subclonal, is_subclonal_maj, is_subclonal_min, chr.segs, chr.names, tumourname, ylim = 5) {
    ...
    if (sum(is_subclonal) > 0) {
      ...
      segments(x0 = subcl_min, y0 = subclones$nMaj1_A[is_subclonal] +
        subclones$nMin1_A[is_subclonal] +
        0.1,
               x1 = subcl_max, y1 = subclones$nMaj1_A[is_subclonal] +
          subclones$nMin1_A[is_subclonal] +
          0.1, col = "#E69F00", pch = "|",
      ...
      segments(x0 = subcl_min, y0 = subclones$nMaj2_A[is_subclonal] +
        subclones$nMin2_A[is_subclonal] +
        0.1,
               x1 = subcl_max, y1 = subclones$nMaj2_A[is_subclonal] +
          subclones$nMin2_A[is_subclonal] +
          0.1, col = "#E69F00", pch = "|",
               lwd = ifelse(is_subclonal_min[is_subclonal], 6 * subclones$frac2_A[is_subclonal], 0), lend = 1)
    }
  ...
  squaresplot <- function(tumourname, run_dir, segment_chr, segment_pos, platform_gamma = 1, pdf = 0, binwidth_baf = 0.25, xylimits = c(-0.2, 5)) {
    ...
    subclone <- subclones[(subclones$chr == segment_chr) &
                            (subclones$startpos <= segment_pos) &
                            (subclones$endpos >= segment_pos),]
    ...
    nMincalc <- (rhopsi$rho -
      1 -
      (subclone$BAF - 1) *
        2^(subclone$LogR / platform_gamma) *
        ((1 - rhopsi$rho) * 2 + rhopsi$rho * rhopsi$psi)) / rhopsi$rho
    nMajcalc <- (rhopsi$rho - 1 + subclone$BAF *
      2^(subclone$LogR / platform_gamma) *
      ((1 - rhopsi$rho) * 2 + rhopsi$rho * rhopsi$psi)) / rhopsi$rho
    ...
    isobafline <- function(nB, cstbaf) {
      (1 - rhopsi$rho + rhopsi$rho * nB -
        cstbaf * (2 - 2 * rhopsi$rho) -
        rhopsi$rho * cstbaf * nB) / (rhopsi$rho * cstbaf)
    }
    ...
    q <- q +
      ggplot2::theme_bw() +
      ggplot2::theme(panel.grid.major = ggplot2::element_line(colour = "darkgrey", size = 0.5), panel.grid.minor = ggplot2::element_blank())
    ...
    if (subclone$frac1_A == 1) {
      ...
      q <- q +
        ggplot2::geom_point(data = solutions, aes(nMaj, nMin, size = frac, colour = factor(sol)), alpha = 0.75, position = ggplot2::position_jitter(width = .05, height = .05), shape = 79) +
        ggplot2::scale_size_continuous(guide = F, limits = c(0, 1), range = c(2, 10)) +
  ...
  totalcn_chrom_plot = function(samplename, subclones, logr, outputfile, purity) {
    ...
    for (i in (1:nrow(subclones))) {
      print(i)
      sel = which(logr$Chromosome == subclones$chr[i] &
                    logr$Position >= subclones$startpos[i] &
                    logr$Position <= subclones$endpos[i])
